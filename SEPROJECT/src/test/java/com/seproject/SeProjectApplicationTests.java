...
import com.seproject.controller.PapersController;
...
class SeProjectApplicationTests {
   ...
   @Test
    ...
    @Test
    void testAssignReview() {
        // Mock data
        int id = 1;
        String review = "This is a sample review";
        Paper mockPaper = new Paper();
        when(paperService.getPaperDetailsById(id)).thenReturn(mockPaper);

        // Call the service method
        ModelAndView result = papersController.assignReviewer(id, review);


        // Assertions
        assertEquals("redirect:/dashboard/getPapers/" + id, result); // Check if the redirection URL is correct
        assertEquals(review, mockPaper.getReview()); // Check if the review was updated correctly
    }
}
